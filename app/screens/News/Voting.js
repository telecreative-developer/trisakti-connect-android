...
class Voting extends Component<{}, State> {
	...
	async handleReadPolls(item) {
		...
		await this.props.checkHasVoted(
					...
					<CardItem>
						<Body>
							...
							<View style={styles.viewFoot}>
								<Text note style={styles.defaultDate}>
						...
						<Right>
							<Thumbnail
								square
								large
								source={{ uri: item.thumbnail_poll }}
...
const mapDispatchToProps = dispatch => {
	return {
		...
		checkHasVoted: (iduser, idpoll, accessToken) =>
			dispatch(checkHasVoted(iduser, idpoll, accessToken)),
		fetchPollsAnswers: (id, accessToken) => dispatch(fetchPollsAnswers(id, accessToken)),
		fetchPollsAnswersRealtime: (id, accessToken) =>
			dispatch(fetchPollsAnswersRealtime(id, accessToken))
	}
}
