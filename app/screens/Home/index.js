...
import { Container, Header, Body, Right, Button, Icon, Footer, FooterTab } from 'native-base'
...
type State = {
  active: number,
  activePageFirst: boolean,
  activePageSecond: boolean,
  activePageThird: boolean,
  activePageFourth: boolean,
  activePageFifth: boolean,
  title: string
}
...
class Home extends Component<{}, State> {
  ...
  renderContent() {
		...
		if(active === 2) {
			return (
				<Chats toSearch={() => this.navigateToSearch()} />
			)
		}else if(active === 3) {
			return (
				<News />
