...
import { StyleSheet, View, Alert, Image, BackHandler, Dimensions, TouchableHighlight } from 'react-native'
...
class Register extends Component {
	...
	async handleCheckNim() {
		if(isEmpty(this.state.nim)) {
			...
			await this.props.setLoading({condition: true, process_on: 'fetch_user_with_nim'})
			try {
				let response = await fetch(`${url}/users?nim=${this.state.nim}&password=1`, {
					method: 'GET',
					headers: {
			...
			return (
				<Container style={styles.container}>
					<View style={{flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
						<H2 style={{fontWeight: 'bold', marginBottom: 20, fontFamily: 'SourceSansPro'}}>Thank You for Register!</H2>
		...
		if(this.state.registerPage === 'register1') {
			return (
				<Container style={styles.container}>
					...
					<Header>
						<Left>
							<Button transparent onPress={() => this.setState({registerPage: ''})}>
					...
					<Content>
						<Form>
							...
							<Item stackedLabel>
								...
								<Input style={styles.input} keyboardType='email-address' value={this.state.email} onChangeText={(email) => this.setState({email})} />
							</Item>
							<Item stackedLabel>
							...
							<Item stackedLabel>
								...
								<Input style={styles.input} value={this.state.kecamatan} onChangeText={(kecamatan) => this.setState({kecamatan})} />
							</Item>
							<Item stackedLabel>
								<Label style={styles.label}>Urban Village</Label>
								<Input style={styles.input} value={this.state.kelurahan} onChangeText={(kelurahan) => this.setState({kelurahan})} />
							</Item>
							<Item stackedLabel>
								<Label style={styles.label}>Province</Label>
								<Input style={styles.input} value={this.state.provinsi} onChangeText={(provinsi) => this.setState({provinsi})} />
							</Item>
							<Item stackedLabel>
								<Label style={styles.label}>Postcode</Label>
								<Input style={styles.input} keyboardType='numeric' value={this.state.postcode} onChangeText={(postcode) => this.setState({postcode})} />
							</Item>
							<Item stackedLabel onPress={() => this.setState({isDateTimePickerVisible: true})}>
								<Label style={styles.label}>Date of Birth</Label>
								<Input style={styles.input} disabled value={moment(this.state.birth).format('LL')} onChangeText={(birth) => this.setState({birth})} />
							</Item>
							<Item stackedLabel>
								<Label style={styles.label}>Place of Birth</Label>
								<Input style={styles.input} value={this.state.birth_place} onChangeText={(birth_place) => this.setState({birth_place})} />
							</Item>
		...
		if(this.state.registerPage === 'register2') {
			return (
				<Container style={styles.container}>
					<Header>
						<Left>
							<Button transparent onPress={() => this.setState({registerPage: 'register1'})}>
								<Icon name='arrow-back' />
							</Button>
						</Left>
						<Body>
					...
					<Content>
						<Form>
							<Item stackedLabel>
								...
								<Input style={styles.input} keyboardType='numeric' value={this.state.graduated} onChangeText={(graduated) => this.setState({graduated})} />
							</Item>
							<View style={{margin: 15}}>
								<Text note style={{color: '#0e0e0e', fontFamily: 'SourceSansPro', fontSize:14}}>Pilih Fakultas</Text>
								<Picker
									mode="dropdown"
							...
							<View style={{margin: 15}}>
								<Text note style={{color: '#0e0e0e', fontFamily: 'SourceSansPro', fontSize: 14}}>Pilih Jurusan</Text>
								<Picker
									mode="dropdown"
							...
							<View style={styles.viewButton}>
								<Button block onPress={() => this.setState({registerPage: 'thankyou'})}>
									{(this.props.loading.condition) ? ( <Spinner color='white' /> ) : ( <Text style={styles.label}>Register</Text> )}
								</Button>
							</View>
						</Form>
					</Content>
					...
					<Content>
						<Form>
							{(this.state.subjectStatus) ? (
								...
								<View style={{margin: 15}}>
									<Text note style={{color: '#0e0e0e'}}>Subject</Text>
									<Picker
										mode="dropdown"
										...
										<Item label='I forgot my email or password' value='I forgot my email or password' />
							...
							<Item stackedLabel style={{marginTop: 10, marginBottom: 10}}>
								...
								<Input value={this.state.contentreport} onChangeText={(contentreport) => this.setState({contentreport})} multiline numberOfLines={4} />
							</Item>
						</Form>
						<View style={{margin: 10, marginLeft: width / 4, marginRight: width / 4}}>
							<Button block style={{borderRadius: 5}} onPress={this.handleSendReport}>
								{(this.props.loading.condition) ? ( <Spinner color='white' /> ) : ( <Text>Send Report</Text> )}
							</Button>
						</View>
					</Content>
				</Container>
			...
			<Container style={styles.container}>
				...
				<View style={styles.viewContainer}>
					<View style={{marginBottom: 15}}>
						<Text style={{fontSize: 12, fontFamily: 'SourceSansPro'}}>*Do not follow the number '0' in front of your NIM</Text>
					</View>
				...
				<Footer style={{backgroundColor: 'transparent'}}>
					<TouchableHighlight onPress={this.handlePressReport}>
						<Text style={{fontFamily: 'SourceSansPro', fontSize: 16}}>Have problem with register and login? Send report.</Text>
					</TouchableHighlight>
				</Footer>
			</Container>
		)
...
export default connect(mapStateToProps, mapDispatchToProps)(ThemeContainer(Register))